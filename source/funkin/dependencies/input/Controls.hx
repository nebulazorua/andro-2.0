package funkin.dependencies.input;
...
class Controls
{
	...
	private static var actions:Map<String, Array<Key>> = [
		...
		"up" => [Keyboard.UP, Keyboard.W, Keyboard.K],
		"right" => [Keyboard.RIGHT, Keyboard.D, Keyboard.L],
		"pause" => [Keyboard.ESCAPE],
		"reset" => [Keyboard.R],
		"back" => [Keyboard.ESCAPE],
		"accept" => [Keyboard.ENTER, Keyboard.Z],
		"botplay" => [Keyboard.F6]
	];
	...
	public static function getJustPressedAction(name:String):Bool
	{
		if (actions.exists(name))
		{
			var keys:Array<Int> = actions.get(name);
	...
	public static function getJustReleasedAction(name:String):Bool
	{
		if (actions.exists(name))
		{
			var keys:Array<Int> = actions.get(name);
